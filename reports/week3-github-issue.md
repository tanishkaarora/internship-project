...
- [x] **At least 15 GitHub commits** total on main. (Currently at 26 commits on main).
...
In this milestone, I implemented the **Smart Column Detective** mini-extension. This component maps conversational queries (e.g., *"Which product drives the most quantity sold?"*) to the actual columns in the uploaded CSV (e.g., `product`, `quantity`). It replaces the rigid fallback of selecting the first column.

#### Evidence of Working Mini-Extension

The screen recording showing the application loading sample datasets, navigating between dashboards, and running a column detective query is stored locally and on GitHub:
* **Recording Link:** [demo/copilot_demo.webp](https://github.com/tanishkaarora/retailbrain/blob/main/demo/copilot_demo.webp)

##### UI Screenshots:
* **Overview Tab:** Shows KPI cards and text-based data profiles of the loaded CSV.
  ![Overview Tab](https://github.com/tanishkaarora/retailbrain/raw/main/docs/images/overview_tab.png)

* **Charts Tab:** Shows auto-generated interactive Plotly charts.
  ![Charts Tab](https://github.com/tanishkaarora/retailbrain/raw/main/docs/images/charts_tab.png)
...
* **Ask Tab (Smart Column Detective Mapping):** Note the information box starting with `🕵️` that details which columns were dynamically mapped.
  ![Ask Tab Result](https://github.com/tanishkaarora/retailbrain/raw/main/docs/images/ask_tab_result.png)

---

### 🧪 Unit Tests Verification

I added unit tests in [test_column_detective.py](https://github.com/tanishkaarora/retailbrain/blob/main/tests/test_column_detective.py) to test the rule-based keyword mapping and LLM fallback mapping behaviors of the `SmartColumnDetective`.

Running [run_tests.py](https://github.com/tanishkaarora/retailbrain/blob/main/run_tests.py) produces the following passing test output:
...
I added 2 more ADRs to the repository:
- 📝 **[ADR-001 (LangGraph Adoption)](https://github.com/tanishkaarora/retailbrain/blob/main/docs/adr/ADR-001.md)** — Architectural Decision Record on migrating to LangGraph StateGraph.
- 📝 **[ADR-002 (FAISS vs ChromaDB)](https://github.com/tanishkaarora/retailbrain/blob/main/docs/adr/ADR-002.md)** — Choosing FAISS over ChromaDB to minimize installation compile issues on Windows and keep setups under 20 minutes.
- 📝 **[ADR-003 (Streamlit Monolith vs API+SPA)](https://github.com/tanishkaarora/retailbrain/blob/main/docs/adr/ADR-003.md)** — Choosing Streamlit as a unified monolith for rapid prototyping and state management vs a decoupled FastAPI + React architecture.
...
#### 1. What's Done
* Implemented the `SmartColumnDetective` in [column_detective.py](https://github.com/tanishkaarora/retailbrain/blob/main/src/analytics/column_detective.py) with fast rule-based paths and robust LLM fallbacks.
